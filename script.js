...
// Add your extra specific IDs here
const extraHoverIDs = ['logodiv','imagediv']; // Add as needed
...
document.addEventListener('mouseover', (e) => {
  ...
  const isDynamicKey =
    el.tagName === 'P' &&
    (el.closest('#colldiv') || el.closest('#typediv') || el.closest('#photodiv'));

  const isExtraHoverID = extraHoverIDs.includes(el.id);

  const isVisible =
    window.getComputedStyle(el).visibility !== 'hidden' &&
    window.getComputedStyle(el).display !== 'none';

  if ((isDynamicKey || isExtraHoverID) && isVisible) {
